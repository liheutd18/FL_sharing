...
for T=1:24
    if(+newload(T)+Esd(T)<0)
        ...
        optMGOpro(T)=sum((newload(T,:)-pv(T,:)).*finalprices(T,:))+pgb(T)*(-newload(T)-Esd(T));
    else
        %optMGOpro(T)=price(T,1)*optEsell(T)-price(T,2)*optEbuy(T)+pgs(T)*(-optNLt(T));
        optMGOpro(T)=sum((newload(T,:)-pv(T,:)).*finalprices(T,:))+pgs(T)*(-newload(T)-Esd(T));
    end
end
...
Benefit_com = [pro_agent benefit_per];
