function net_new = weightsupdate(networks,numHiddenUnits)

%%%% learning rate eta
eta_FL = 0.001;
%%%% shorten networks
networks = {networks{1} networks{2} networks{6} };
...
%%%% layers_new_all is the global net
    ...
    layers_new_all(2,1).Bias = single(zeros(numHiddenUnits*4,1));
    weights_agg_new_all{1} = [layers_new_all(2,1).InputWeights, layers_new_all(2,1).RecurrentWeights, layers_new_all(2,1).Bias];
...
for iter = 2:iter_max    
    ...
    for i = 1:length(networks)
        ...
        layers_new_all(2,1).Bias = layers_new_all(2,1).Bias + Weights_table{i}(:,numHiddenUnits+2);
    end
    ...
    layers_new_all(2,1).Bias = layers_new_all(2,1).Bias/length(networks);
    
    
    weights_agg_new_all{iter} = [layers_new_all(2,1).InputWeights, layers_new_all(2,1).RecurrentWeights, layers_new_all(2,1).Bias];
